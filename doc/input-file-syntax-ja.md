...
|`shift_divisor`|integer|__必須。__ ~~正の数。~~ Extendedのゲーム内で、時刻表のshift値が1ヶ月を何分割するか。コンフィグの`spacing_shift_divisor`の値。|
...
* 駅の形式は、`駅名`あるいは`駅名@ID`です。IDはinteger形式です。IDを省略した場合は`0`となります。
...
|プロパティ名|値の形式|プロパティの意味|
|------------|--------|----------------|
|`time_id`|integer|デフォルト値0。同じ区間でも所要時間を区別しなければならない場合に、これを別の値にします。なお、同じ区間と`time_id`を持つものが複数の行に存在しても、正常に登録されます。|
...
|`load`|time|この駅に到着してからの乗降時間。省略された場合は路線やファイル全体のデフォルト値が用いられます。次の駅が同じ駅（通常、IDが異なる）の場合には、指定に関わらず強制的に0になります。|
...
### 発車時刻の計算
乗降時間(`wait`)と(指定された場合のみ)折り返し時間(`reverse`)のうちの長い方を基にして、必要な停車時間が計算されます。
`shift`、`shift_num`、`wait`により発車時刻が指定された場合、必要な停車時間を満たす、一番近い発車時刻が用いられます。
指定されない場合、必要な停車時間を基にして自動的に発車時刻が計算されます。
